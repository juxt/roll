...
Roll is about the deployment of application services, providing an opinionated description of what services will commonly need. Roll will provision you a secure Beanstalk like AWS rolling blue/green infrastructure using Terraform, ClojureScript, Aero and Lumo.

Roll operates on a per environment basis, so you have a different Roll coniguration data structure (an EDN file) per environment.

Roll assumes that deployable application versions are accessible in S3, and you get to provide the start-up instructions. Therefore it's orthogonal to whether you like to use Uberjars, Docker containers, or whatever else.

Warning: Roll is in active development, everything about it - including this document - is being modified on a regular basis.
...
Roll will preprocess the Roll configuration you give it. An example of pre-processing Roll deriving the most recent application version based on the contents of the S3 releases bucket, so you don't have to hardcode the application version manually.

After the pre-processing step you can hook and it manipulate the data-structure as you wish.

2) Create TF data-structure.
...
To use Roll from the command line, we suggest you use it in conjunction with https://github.com/juxt/mach[Mach] (Mach is your Make).

Details on Mach integration will be added soon.
